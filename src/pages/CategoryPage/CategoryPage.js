  ...
  const Item = ({ item }) => (
    <View className=" min-h-64 border border-gray-300 rounded-md bg-white dark:bg-slate-900 dark:border-gray-600 relative"
      style={{ width:(width/2)-16,margin:6 }}>
       ...
       <TouchableOpacity onPress={()=>navigation.navigate('Single page',{item:item})}
  ...
  return (
    <SafeAreaView className='flex-1 bg-[#fff] dark:bg-black items-center justify-between' style={{ width: width, height: height }}>
      ...
      <ScrollView nestedScrollEnabled={true}>
        ...
        <View className="flex-row items-center justify-start p-3 rounded-xl bg-[#ffff6e] dark:bg-slate-900 dark:border-gray-600 dark:border"
        style={{ width:width-16,margin:6 }}>
            <Image className="w-2/6 h-20 rounded-lg" source={{ uri: data.image }} alt='banner' /> 
            <View className="w-4/6">
              <Text className="text-2xl dark:text-white px-2"
                  style={{ fontFamily: theme.fonts.bold }}>{data.title}
              </Text>  
            </View>    
        </View>
